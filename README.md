...
The system determines when flashcards should next be reviewed, based on user feedback, while enforcing **deterministic growth, idempotency & data durability**.
...
This project uses a modular and testable architecture designed for clarity & maintainability. All core logic is organized under the `scheduler/` Django app, with isolated layers for API, data, services & configuration.
...
- **Transparent:** Easy to log, debug & test.
